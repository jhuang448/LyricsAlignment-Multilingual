...
def predict_align(args, model, test_data, device, model_type):
    ...
    with tqdm(total=len(test_data)) as pbar, torch.no_grad():
        for example_num, _data in enumerate(dataloader):
            ...
            if os.path.exists(os.path.join(args.pred_dir, audio_name + "_align.csv")):
                print("Skip", audio_name)
                pbar.update(1)
                continue
...
def predict_w_bdr(args, ac_model, bdr_model, test_data, device, alpha, model_type):
    ...
    with tqdm(total=len(test_data)) as pbar, torch.no_grad():
        for example_num, _data in enumerate(dataloader):
            ...
            # reframe bdr audio 
            x_bdr = x_bdr.reshape(1,1,-1)
            x_bdr = utils.move_data_to_device(x_bdr, device)
            x_bdr = x_bdr.squeeze(0)
            x_bdr = x_bdr.squeeze(1)
            x_bdr = train_audio_transforms.to(device)(x_bdr)
            x_bdr = nn.utils.rnn.pad_sequence(x_bdr, batch_first=True).unsqueeze(1)

            # get boundary prob curve
            bdr_outputs = bdr_model(x_bdr).data.cpu().numpy().reshape(-1)
